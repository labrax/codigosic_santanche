...
void runIterations(int amount_interactions)
{
...
int main(int argc, char * argv[])
{
	...
	while(run == 1)
	{
		...
		printf("1. print grid; 2. change element; 3. run iterations; 4. exit\n");
		scanf(" %d", &option);

		int x, y, trait_num, new_value, amount;
		switch(option)
		{
			case 1:
				printGrid();
				break;
			case 2:
				printf("Enter x_pos, y_pos, trait_num, new_value\n");
				
				scanf(" %d %d %d %d", &x, &y, &trait_num, &new_value);
				if(x >=0 && y >= 0 && x < GRID_SIZE && y < GRID_SIZE && trait_num >= 0 && trait_num < AMOUNT_TRAITS && new_value >= 0 && new_value < MAX_TRAITS_VALUE)
				{
					grid[y][x]->traits[trait_num] = new_value;
					grid[y][x]->changed = 1;
				}
				break;
			case 3:
				printf("Enter amount iteractions: ");
				scanf(" %d", &amount);
				runIterations(amount);
				break;
			case 4:
				run = 0;
				break;
...
/*	printf("Initial grid:\n");
	...
	runIterations(amount_interactions);
